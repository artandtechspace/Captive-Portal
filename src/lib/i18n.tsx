...
const getInitialLanguage = (): LanguageCode => {
  if (typeof window !== 'undefined') {
    const stored = window.localStorage.getItem('language') as LanguageCode | null;
    if (stored && supportedLanguages.includes(stored)) {
      return stored;
    }

    const navigatorLanguage = window.navigator.language?.split('-')[0] as LanguageCode | undefined;
...
interface TranslationContextValue {
  ...
  t: (key: string) => TranslationValue | undefined;
}
...
export const TranslationProvider = ({ children }: { children: ReactNode }) => {
  ...
  const dictionary = useMemo(() => translations[language] ?? translations.de, [language]);

  const languageOptions = useMemo<LanguageOption[]>(() => {
    const currentDictionary = translations[language] ?? translations.de;
    return supportedLanguages.map((code) => {
      const names = currentDictionary.languages ?? {};
      const fallback = translations[code]?.languages ?? {};
      return {
        value: code,
        label: names[code] ?? fallback[code] ?? code
      };
    });
  }, [language]);
  ...
  const t = useCallback(
    (key: string): TranslationValue | undefined => {
      return getNestedValue(dictionary, key) ?? getNestedValue(translations.de, key);
    },
    [dictionary]
  );

  const setLanguage = useCallback((code: string) => {
    if (supportedLanguages.includes(code as LanguageCode)) {
  ...
  const value = useMemo<TranslationContextValue>(
    ...
    [language, setLanguage, languageOptions, t]
  );
