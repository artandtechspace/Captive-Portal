...
import { CaptivePortalClient, ClientState, type LogonRequest } from 'src/lib/api';
          ...
          <CardContent className="space-y-6">
              ...
              <form className="space-y-5" onSubmit={login} autoComplete="off">
                ...
                <div className="flex items-start gap-3 rounded-lg bg-muted/40 p-3">
                  <Checkbox
                    ...
                    required
                  />
                  <Label htmlFor="termsCheckbox" className="text-sm font-normal leading-6 text-muted-foreground">
            ...
            {showAnonymous && (
              <Button className="w-full" onClick={loginAnonymous} disabled={busy}>
                {busy ? <Loader2 className="h-4 w-4 animate-spin" aria-hidden /> : translateString('anonymousButton', 'Sign in anonymously')}
              </Button>
            )}

            {showLogout && (
              <Button className="w-full" onClick={logout} disabled={busy}>
                {busy ? <Loader2 className="h-4 w-4 animate-spin" aria-hidden /> : translateString('logoutButton', 'Sign out')}
              </Button>
            )}
          </CardContent>
        </Card>
